...
function applyMonacoConfiguration(root?: Node) {
  ...
  if (
    (
      (root as JSONObject)?.members?.find(
        (x) => x.name === "applyMonacoConfiguration"
      )?.value as JSONBoolean
    )?.value
  ) {
    for (const member of (root as JSONObject).members) {
      if (!(member.name in configuration)) {
        ...
        diagnostics.push({
          ...
          endColumn:
            member.value.position!.start.column + 2 + member.name.length,
        });
      }
    }
    monaco.editor.setModelMarkers(editor.getModel()!, "json", diagnostics);
...
function hasDuplicates(name: string, root: Node) {
  const duplicatesArray = (root as JSONObject)?.members?.filter(
    (x) => x.name === name
  );
